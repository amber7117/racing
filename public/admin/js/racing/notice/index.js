...
$(function () {
    table = $('#noticeTable').DataTable({
        ...
        start: 0,  //起始数
        ajax: {
            "url": "/admin/racing/notice/searchPage",
            type: 'GET', // 默认get吧
            "data": function (data) {
        ...
        "columns": [
            {"data": null, "sClass": "text-center", "width": "33%"},
            {"data": "title", "sClass": "text-center", "width": "34%"},
            {
                "data": "id", "sClass": "text-center", "width": "33%",
                "render": function (data, type, full) {
                    var btns = [];
                    var editBtn ='<a href="javascript:;"  onclick="update(\'/admin/racing/notice/update?id='+data+'\');return false;" title="编辑" class="btn default btn-xs"><i class="fa fa-edit"></i>编辑</a>';
                    btns.push(editBtn);
...
function del(id) {
    if (confirm('您确定要删除吗？')) {
        $.ajax({
            url: '/admin/racing/notice/delete',
            type: 'post',
            async: false,
            data: {
                'id': id
